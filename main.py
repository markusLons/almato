...
class RailroadStationApp(QMainWindow):
    def __init__(self):
        ...
        # Создаем временную шкалу с интервалом каждые 5 минут
        time_scale_layout = QHBoxLayout()
        for hour in range(24):
            for minute in range(0, 60, 5):
                time_label = QLabel(f"{hour:02d}:{minute:02d}")
                time_label.setFixedSize(60, 50)  # Уменьшаем размер метки времени
                time_scale_layout.addWidget(time_label)

                # Добавляем вертикальную линию между 5-минутными интервалами (кроме последнего интервала)
                if minute < 55:
                    line = QFrame(self.scroll_content)
                    line.setFrameShape(QFrame.VLine)
                    line.setFrameShadow(QFrame.Sunken)
                    line.setLineWidth(2)  # Увеличиваем жирность линии
        ...
        for _ in range(6):
            ...
            line.setFixedSize(25* 50* 24 , 50)  # Устанавливаем размер кнопки равный ширине временной шкалы
...
class DraggableButton(QPushButton):
    def __init__(self, image_path, parent=None, horizontal_lines=None):
